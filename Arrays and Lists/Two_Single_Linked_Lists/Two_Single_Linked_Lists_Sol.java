...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        SLLNode<Card> p = firstSorcererCards.getFirst();
		SLLNode<Card> q = secondSorcererCards.getFirst();
		SLLNode<Card> max = p;

		while (p != null) {
			if (p.element.significance() > max.element.significance()) {
				max = p;
			}
			p = p.succ;
		}
		...
		secondSorcererCards.insertBefore(max.element, q);
